...
function CompactMatchRow({ match, perspectivePlayerId, legIndex, totalLegs, onMatchUpdated }: {
  match: LeagueMatch;
  ...
  return (
    <>
      <div className={`flex items-center justify-between px-2 py-1.5 rounded-lg border ${result.bg}`}>
        ...
        <div className="flex items-center gap-3">
          <span className={`text-sm font-bold ${result.color}`}>{myScore} – {oppScore}</span>
          ...
          {group.matchups.map((matchup) => (
            <Card key={matchup.opponentName} variant="glass" className="!p-2.5 sm:!p-3">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-bold text-light-900 dark:text-white">vs {matchup.opponentName}</span>
                {matchup.matches.length > 1 && (
                  <span className="text-[10px] text-light-500 dark:text-gray-500">{matchup.matches.length} legs</span>
                )}
              </div>
              <div className="space-y-1.5">
                {matchup.matches.map((match, legIndex) => (
                  <CompactMatchRow
                    key={match.id}
                    match={match}
                    perspectivePlayerId={group.playerId}
                    legIndex={legIndex}
                    totalLegs={matchup.matches.length}
                ...
                <div className="space-y-1.5">
                  {group.matches.map((match, legIndex) => (
                    <CompactMatchRow
                      ...
                      legIndex={legIndex}
                      totalLegs={group.matches.length}
                      onMatchUpdated={onMatchUpdated}
                    />
                  ))}
                </div>
