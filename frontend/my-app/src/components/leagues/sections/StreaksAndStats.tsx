        ...
        {currentStreaks.length > 0 ? (
          ...
          <Card variant="glass">
            <div className="text-center py-12">
              ...
              <p className="text-gray-400">No active win streaks</p>
              <p className="text-sm text-gray-500 mt-1">Win consecutive matches to start a streak!</p>
            </div>
          </Card>
        )}
      </div>
        ...
        {allTimeBestStreaks.length > 0 && allTimeBestStreaks[0].longestStreak > 0 ? (
          ...
          <Card variant="glass">
            <div className="text-center py-12">
              ...
              <p className="text-gray-400">No streaks recorded yet</p>
            </div>
          </Card>
        )}
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {highestWinRate && (
              <Card variant="glass" className="bg-gradient-to-br from-green-500/20 to-emerald-600/20 border-green-500/30">
                <div className="text-center">
                  <p className="text-xs text-gray-400 mb-2">Highest Win Rate</p>
                  <p className="text-xl font-bold text-white mb-1">{highestWinRate.name}</p>
            ...
            {topScorer && (
              <Card variant="glass" className="bg-gradient-to-br from-cyber-500/20 to-cyber-600/20 border-cyber-500/30">
                <div className="text-center">
                  <p className="text-xs text-gray-400 mb-2">Most Goals Scored</p>
            ...
            {bestDefense && (
              <Card variant="glass" className="bg-gradient-to-br from-electric-500/20 to-electric-600/20 border-electric-500/30">
                <div className="text-center">
                  <p className="text-xs text-gray-400 mb-2">Best Defense</p>
                  <p className="text-xl font-bold text-white mb-1">{bestDefense.name}</p>
                  <p className="text-2xl font-black text-electric-400">{bestDefense.goalsAgainst || 0}</p>
                  <p className="text-xs text-gray-400 mt-1">goals conceded</p>
                </div>
              </Card>
            )}
          </div>
