                    ...
                    >
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-light-900 dark:text-white truncate">{team.name}</p>
                          {team.psnId && (
                            <p className="text-xs text-light-600 dark:text-gray-400 truncate">PSN: {team.psnId}</p>
                          )}
                        </div>
                      </div>
                      ...
                      {isAuthenticated && tournament.groups && tournament.groups.length > 0 && team.id && (
                        <div className="mt-2 pt-2 border-t border-white/10">
                          <select
                            ...
                            className="w-full px-3 py-2 text-xs bg-dark-100/50 border-2 border-white/10 rounded-tech text-white focus:outline-none focus:border-cyber-500 transition-all backdrop-blur-sm hover:border-cyber-500/50"
                          >
                            {tournament.groups.map((group) => (
                              <option key={group.id} value={group.id} className="bg-dark-100 text-black">
                                Move to {group.name}
                              </option>
                            ))}
                          </select>
