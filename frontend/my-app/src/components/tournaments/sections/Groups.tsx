  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-lg sm:text-2xl font-bold text-light-900 dark:text-white mb-0.5">Group Stage</h2>
          <p className="text-xs sm:text-sm text-light-600 dark:text-gray-400">{tournament.groups.length} Groups</p>
        </div>
        <div className="bg-light-200/50 dark:bg-dark-100/50 backdrop-blur-md border border-black/10 dark:border-white/10 rounded-tech px-3 py-1.5 sm:px-4 sm:py-2 flex-shrink-0">
          <div className="flex items-center gap-1.5">
            <TrendingUp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-cyber-400" />
            <span className="text-[11px] sm:text-sm text-light-600 dark:text-gray-400">
    ...
    >
      <Card variant="glass" className={`bg-gradient-to-br ${gradient} border-white/10`}>
        ...
        <div className="flex items-center justify-between mb-2 sm:mb-4">
          <h3 className="text-base sm:text-xl font-bold text-light-900 dark:text-white">{group.name}</h3>
          <div className="text-right">
            <div className="text-[11px] sm:text-sm text-light-600 dark:text-gray-400 font-medium mb-1">
              {playedMatches}/{totalMatches}
            </div>
            <div className="w-16 sm:w-24 bg-light-300 dark:bg-dark-200 rounded-full h-1.5 sm:h-2">
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${completion}%` }}
                className="bg-gradient-to-r from-cyber-400 to-electric-400 h-1.5 sm:h-2 rounded-full"
              />
            </div>
          </div>
        </div>
        ...
        <div className="mb-2 sm:mb-6">
          <h4 className="text-[11px] sm:text-sm font-semibold text-light-700 dark:text-gray-300 mb-1.5 sm:mb-3">Standings</h4>
            ...
            {sorted.map((standing, i) => {
              ...
              return (
                <div
                  ...
                  className="flex items-center justify-between bg-light-200/50 dark:bg-dark-100/50 backdrop-blur-sm rounded-tech p-1.5 sm:p-3 border border-black/5 dark:border-white/5 gap-1 sm:gap-2"
                >
                  <div className="flex items-center gap-1.5 sm:gap-3 min-w-0 flex-1">
                    <div
                      className={`w-5 h-5 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-bold flex-shrink-0 ${
                        i < 2
                    ...
                    <span className="font-semibold text-light-900 dark:text-white truncate text-[11px] sm:text-sm">{standing.teamName}</span>
                  </div>
                  <div className="flex items-center gap-1 sm:gap-3 text-[10px] sm:text-xs flex-shrink-0">
                    <div className="flex items-center gap-0.5 sm:gap-1">
                      <span className="font-bold text-cyber-400">{standing.points}<span className="hidden sm:inline">pts</span></span>

                      {/* Adjustment Badge */}
                      {hasAdjustments && (
                        <button
                          onClick={() => handleOpenHistoryModal(standing.teamName, standing.pointAdjustments!)}
                          className={`px-1 py-0.5 rounded text-[9px] sm:text-[10px] font-bold transition-all hover:scale-110 active:scale-95 ${
                            totalAdjustment > 0
                    ...
                    <span className="text-yellow-400">{standing.drawn}D</span>
                    ...
                    {isAuthenticated && (
                      <button
                        ...
                        className="p-1 sm:p-1.5 rounded-lg bg-cyber-500/10 hover:bg-cyber-500/20 border border-cyber-500/30 hover:border-cyber-500/50 transition-all group"
                        title="Adjust points"
                      >
                        <Settings className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-cyber-400 group-hover:text-cyber-300 group-hover:rotate-90 transition-all" />
                      </button>
                    )}
                  </div>
                </div>
